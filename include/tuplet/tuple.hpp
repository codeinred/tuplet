...
constexpr inline bool _impl_less(
    ...
    type_list<B2...>) {
    bool is_less = false;
    (_partial_cmp(t1.identity_t<B1>::value, t2.identity_t<B2>::value, is_less)
     && ... && true);
    return is_less;
}
...
struct type_map : Bases... {
    ...
    auto operator!=(type_map const& other) const { return !(*this == other); }
    auto operator<(type_map const& other) const {
...
struct tuple : tuple_base_t<T...> {
   ...
   private:
    ...
    constexpr void assign_op_impl(U&& u, type_list<B1...>, type_list<B2...>) {
        // See:
    ...
    constexpr void assign_op_impl_indices(U&& u, std::index_sequence<I...>) {
        using std::get;
...
struct pair {
    ...
    template <TUPLET_OTHER_THAN(pair, Type)> // Preserves default assignments
